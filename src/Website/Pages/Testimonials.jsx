...
// Single Avatar Component to handle error fallback smoothly
const ClientAvatar = ({ clientImage, clientName, backendUrl }) => {
  ...
  const getImageUrl = (img) => {
    ...
    const cleanBackend = backendUrl ? backendUrl.replace(/\/$/, '') : '';
    const cleanImgPath = img.startsWith('/') ? img : `/${img}`;
    return `${cleanBackend}${cleanImgPath}`;
  };
  ...
  const imgUrl = getImageUrl(clientImage);

  if (!imgUrl || hasError) {
    return (
      <div className="w-14 h-14 rounded-full bg-gradient-to-br from-orange-500 to-amber-600 flex items-center justify-center text-white font-black text-lg shadow-lg shadow-orange-500/20 border-2 border-orange-500/40 flex-shrink-0">
        {initials}
      </div>
    );
  }
  ...
  return (
    <div className="relative w-14 h-14 flex-shrink-0">
      <img
        ...
        className="w-14 h-14 rounded-full object-cover border-2 border-orange-500/40 shadow-md shadow-black/40"
        loading="lazy"
...
export const Testimonials = () => {
  ...
  return (
    <div
      className="w-full min-h-screen bg-[#070707] pt-32 pb-24 overflow-hidden relative"
                ...
                <div className="flex gap-1 mb-6">
                  {Array.from({ length: 5 }, (_, s) => (
                    <svg
                      ...
                      className={`h-5 w-5 ${s < (t.rating || 5) ? 'text-orange-500' : 'text-white/10'}`}
                      fill="currentColor"
                      viewBox="0 0 24 24"
                    >
