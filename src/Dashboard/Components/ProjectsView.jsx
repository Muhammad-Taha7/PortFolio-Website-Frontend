...
import { Plus, X, Upload, CheckCircle, Trash2, Image as ImageIcon, ExternalLink, GitBranch, Code, Briefcase, Edit2, FolderOpen } from 'lucide-react';
...
export const ProjectsView = () => {
    ...
    const fetchProjects = async () => {
        try {
            const res = await fetch(`${BACKEND_URL}/api/projects?featured=false`, {
            ...
            <div className="flex justify-between items-center mb-10">
                ...
                {!isAdding && (
                    <button 
                        ...
                        className="px-6 py-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl shadow-lg transition-all font-semibold flex items-center gap-2 hover:-translate-y-1 cursor-pointer"
                    >
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                ...
                                <input type="text" value={name} onChange={(e) => setName(e.target.value)}
                                    ...
                                    className="w-full border-2 rounded-xl py-3 px-4 outline-none focus:border-emerald-500 transition-all" />
                            </div>
                            <div className="space-y-2">
                        ...
                        <div className="space-y-2">
                            ...
                            <textarea rows="4" value={description} onChange={(e) => setDescription(e.target.value)}
                                ...
                                style={{ color: 'var(--text-main)', borderColor: 'var(--border-color)', backgroundColor: 'var(--bg-main)' }}
                                className="w-full border-2 rounded-xl py-3 px-4 outline-none focus:border-emerald-500 transition-all resize-none" />
                        </div>
                        ...
                        <div className="space-y-3 pt-2">
                            <label style={{ color: 'var(--text-main)' }} className="font-semibold text-sm uppercase tracking-wide flex items-center justify-between">
                                Project Images
                                <span className="text-xs text-emerald-400 font-normal normal-case">Max 5 images</span>
                            </label>
                            
                            <div onClick={() => fileInputRef.current?.click()}
                                style={{ backgroundColor: 'var(--bg-main)', borderColor: 'var(--border-color)' }}
                                className="border-2 border-dashed rounded-2xl p-8 flex flex-col items-center justify-center cursor-pointer hover:border-emerald-500/50 hover:bg-emerald-500/5 transition-all text-center">
                            ...
                            <div className="flex gap-2">
                                <input type="url" value={onlineUrlInput} onChange={(e) => setOnlineUrlInput(e.target.value)}
                                    ...
                                    style={{ color: 'var(--text-main)', borderColor: 'var(--border-color)', backgroundColor: 'var(--bg-main)' }}
                                    className="flex-1 border-2 rounded-xl py-3 px-4 outline-none focus:border-emerald-500 transition-all" />
                                <button type="button" onClick={handleAddOnlineUrl}
                                    className="px-6 py-3 bg-emerald-500/10 hover:bg-emerald-500 text-emerald-400 hover:text-white rounded-xl transition-colors font-bold whitespace-nowrap cursor-pointer">
                                    Add Link
                                </button>
                            </div>
                            ...
                            {(existingImages.length > 0 || previewUrls.length > 0 || onlineUrls.length > 0) && (
                                <div className="flex flex-wrap gap-4 mt-4">
                                    ...
                                    {previewUrls.map((url, index) => (
                                        <div key={`file-${index}`} className="relative h-28 w-28 rounded-xl overflow-hidden border-2 border-emerald-500/20 shadow-md group">
                        ...
                        <div className="flex justify-end gap-4 pt-6 border-t" style={{ borderColor: 'var(--border-color)' }}>
                            ...
                            <button type="submit" disabled={status.loading}
                                className="px-8 py-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-bold shadow-lg disabled:opacity-50 transition-all flex items-center gap-2 cursor-pointer hover:-translate-y-1">
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                    {projects.length === 0 ? (
                        <div style={{ borderColor: 'var(--border-color)' }} className="col-span-full py-20 border-2 border-dashed rounded-3xl flex flex-col items-center text-center">
                            ...
                            <h4 style={{ color: 'var(--text-main)' }} className="text-xl font-bold mb-2">No Projects Yet</h4>
                            <p style={{ color: 'var(--text-muted)' }} className="max-w-md">Add projects to build your portfolio collection.</p>
                        </div>
                    ) : (
                            ...
                            <div key={project._id} style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-color)' }} className="flex flex-col border rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 group">
                                ...
                                <div className="p-8 flex-1 flex flex-col">
                                    <div className="flex justify-between items-start mb-4">
                                        ...
                                        <div className="flex gap-2">
                                            {project.githubLink && (
                                                <a href={project.githubLink} target="_blank" rel="noreferrer" className="p-2 text-slate-400 hover:text-emerald-400 hover:bg-emerald-400/10 rounded-xl transition-colors">
                                                    <GitBranch size={20} />
                                                </a>
                                            )}
                                            {project.liveLink && (
                                    ...
                                    <div className="flex flex-wrap gap-2 mb-8">
                                        {project.technologies && project.technologies.map((tech, idx) => (
                                            <span key={idx} className="px-3 py-1 bg-emerald-500/10 text-emerald-400 rounded-lg text-xs font-bold uppercase tracking-wider">{tech}</span>
                                        ))}
                                    </div>
                                    
                                    <div className="pt-6 border-t flex justify-end gap-3" style={{ borderColor: 'var(--border-color)' }}>
                                        <button onClick={() => startEdit(project)}
                                            className="flex items-center gap-2 px-5 py-2.5 text-emerald-400 hover:text-white hover:bg-emerald-500 rounded-xl transition-colors text-sm font-bold cursor-pointer">
                                            <Edit2 size={16} /> Edit
                                        </button>
